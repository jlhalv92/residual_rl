...
from src.algorithm.deep_actor_critic.boosted_residual_rl import BoostedResidualRL
...
def experiment(alg, n_epochs, n_steps, n_episodes_test, run_id):
    ...
    if boosting:
        ...
        agent.setup_residual(prior_agents=[old_agent], use_kl_on_pi=False,kl_on_pi_alpha=0.03, copy_weights=True)
    ...
    agent.save("checkpoint/restnetBoostResidual_unfreeze_kl_0.03_{}".format(run_id))
    wandb.finish()
def sum(x,y, exp):
    print(exp)
...
if __name__ == '__main__':
    ...
    # for i in range(5):
    n_experiment = 5
        # experiment(alg=BoostedResidualRL, n_epochs=150, n_steps=5000, n_episodes_test=10, run_id=i)
    # Parallel(n_jobs=5)()
    # for x in range(5):
    #
    #     for y in range(6):
    out = Parallel(n_jobs=-1)(delayed(experiment)(alg=BoostedResidualRL, n_epochs=150, n_steps=5000, n_episodes_test=10, run_id=experiment_id) for experiment_id in range(n_experiment))
