...
class BHyRLPolicy(Policy):
    ...
    def reset(self):
        self._mu_approximator.model.network.weights_init()
        self._sigma_approximator.model.network.weights_init()
...
class BHyRLPolicy2(Policy):
    ...
    def __init__(self, mu_approximator, sigma_approximator,
                 min_a, max_a, log_std_min, log_std_max, gauss_noise_cov, prior_policy=None):
        """
        Constructor.
...
class BRL(DeepAC):
    ...
    def __init__(self, mdp_info, actor_mu_params, actor_sigma_params,
                 ...
                 gauss_noise_cov=0.01, critic_fit_params=None, prior_policy=None):
        """
        Constructor.
        ...
        policy = BHyRLPolicy2(actor_mu_approximator,
    ...
    def copy_weights_critic(self, old_critic, current_critic):
        ...
        for i in range(len(old_critic)):
            ...
            keys = list(old_state_dict.keys())[:-1]
            print(keys)
            exit()
...
class BRLReset(DeepAC):
    ...
    def __init__(self, mdp_info, actor_mu_params, actor_sigma_params,
                 ...
                 initial_replay_size, max_replay_size, warmup_transitions, tau,
                 lr_alpha, log_std_min=-3, log_std_max=2, use_entropy=False, target_entropy=None,
                 gauss_noise_cov=0.01, critic_fit_params=None, prior_policy=None, update_freq=1):
        """
        Constructor.
    ...
    def fit(self, dataset):
        ...
        for _ in range(self._update_freq):
            if self._replay_memory.initialized:
                ...
                self._critic_approximator.fit(state, action, rho,
                                              **self._critic_fit_params)
                if self.steps == self.update_freq_target:
                    self._update_target(self._critic_approximator,
                                        self._target_critic_approximator)
                    self.steps = 0
