...
from src.networks.networks import CriticNetwork, ActorNetwork, QRESLIM, QRESLIM2,QRESLIM3, QRESLIM4
...
def experiment(alg, n_epochs, n_steps, n_episodes_test, run_id, target_speed, residuals, reference_epoch, curriculum_id, stop_log):
    ...
    critics = [QRESLIM, QRESLIM2, QRESLIM3, QRESLIM4]
    # MDP
    horizon = 500
    gamma = 0.99
    ...
    if log:
        wandb.init(
            ...
            name="residual_resnet_curriculum_unfreeze_no_policy"
        )
    ...
    agent.save("checkpoint/walker_run_curriculum_3_u_np_steps_{}_{}".format(curriculum_id, run_id))
    if stop_log:
        wandb.finish()
...
if __name__ == '__main__':
    ...
    for i in range(3):
        ...
        epochs = [10, 10, 20]
        ref_epochs = [0, 10, 20]
        rhos = ["checkpoint/walker_run_curriculum_3_u_np_steps_{}_{}".format(j, i) for j in range(2)]
